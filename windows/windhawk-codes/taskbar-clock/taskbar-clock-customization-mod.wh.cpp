...
* Modern Graphical Views & Gauges:
  * `%cpu_bar%`, `%cpu_bar5%`, `%cpu_bar8%`, `%cpu_bar10%` - smooth progress bars for CPU.
  * `%cpu_spark%` - rolling history trend graph for CPU (8 samples using block levels ▂▃▄▅▆▇█).
  * `%cpu_badge%` - color badge indicator with CPU usage value (e.g. 🟢 25%, 🟡 65%, 🔴 92%).
  * `%ram_bar%`, `%ram_bar5%`, `%ram_bar8%`, `%ram_bar10%`, `%ram_spark%`, `%ram_badge%` - RAM progress bars, trend graph & badge.
  * `%gpu_bar%`, `%gpu_bar5%`, `%gpu_bar8%`, `%gpu_bar10%`, `%gpu_spark%`, `%gpu_badge%` - GPU progress bars, trend graph & badge.
  * `%vram_bar%`, `%vram_bar5%`, `%vram_bar8%`, `%vram_bar10%`, `%vram_spark%` - VRAM progress bars & trend graph.
  * `%temp_bar%`, `%temp_bar5%`, `%temp_bar8%`, `%temp_bar10%`, `%temp_spark%`, `%temp_badge%` - CPU Temp progress bars, trend graph & thermal badge (❄️ 38°C, 🌡️ 65°C, 🔥 85°C).
  * `%battery_bar%`, `%battery_bar5%`, `%battery_bar8%`, `%battery_bar10%`, `%battery_spark%`, `%battery_badge%` - Battery progress bars, trend graph & power badge.
  * `%net_spark%`, `%disk_spark%` - Network & Disk transfer trend graphs.
  * `%bar:<metric>:<length>:<style>%` - custom progress bar (styles: smooth, block, pill, dot). Example: `%bar:cpu:8:smooth%`.
  * `%spark:<metric>:<length>%` - custom sparkline graph. Example: `%spark:cpu:8%`.
...
- ContainerStyle:
  - Enable: false
    $name: Enable custom container style
  - BackgroundColor: ""
    $name: Background color
    $description: >-
      Hex color string (e.g. #1E1E2EC0 for dark glass, #0F172AEE for solid dark)
  - BorderColor: ""
    $name: Border color
    $description: >-
      Hex color string (e.g. #383E5A or #8070A0FF)
  - BorderThickness: 0
    $name: Border thickness
  - CornerRadius: -1
    $name: Corner radius
    $description: Corner radius in pixels (e.g., 0 for boxy card, 8 for rounded card, 16 for pill box)
  - Height: 0
    $name: Custom height (Windows 11)
    $description: >-
      Custom height in pixels (e.g., 52, 60, 68). Allows the clock card box to extend beyond normal taskbar height. Set to 0 for auto.
  - Width: 0
    $name: Custom width (Windows 11)
    $description: >-
      Custom width in pixels (e.g., 220, 250). Set to 0 for auto.
  - PaddingLeft: -1
    $name: Padding left
  - PaddingTop: -1
    $name: Padding top
  ...
  - PaddingBottom: -1
    $name: Padding bottom
  - IgnoreSystemPadding: false
    $name: Ignore system built-in padding/margins
    $description: Removes default Explorer taskbar margins so custom card container fits perfectly without text clipping.
  - GlassyFinish: false
    $name: Enable glassy finish and rounded hover/click highlights
    $description: Matches parent button click and hover highlight to the exact container corner radius for a sleek glass aesthetic.
  $name: Clock container card style (Windows 11)
...
struct ContainerStyleSettings {
    bool enable;
    StringSetting backgroundColor;
    StringSetting borderColor;
    int borderThickness;
    int cornerRadius;
    int height;
    int width;
...
struct {
    ...
    ContainerStyleSettings containerStyle;
...
// Graphical metrics & history buffers
struct RollingMetricHistory {
    static constexpr size_t kMaxSamples = 10;
    ...
    void Push(double val) {
        if (count < kMaxSamples) {
            ...
            for (size_t i = 0; i < kMaxSamples - 1; i++) {
                samples[i] = samples[i + 1];
            }
            samples[kMaxSamples - 1] = val;
        }
    }
};

struct MetricHistories {
    RollingMetricHistory cpu;
    RollingMetricHistory ram;
    RollingMetricHistory gpu;
    RollingMetricHistory vram;
    RollingMetricHistory temp;
    RollingMetricHistory net;
    RollingMetricHistory disk;
    RollingMetricHistory battery;
} g_metricHistories;

FormattedString<FORMATTED_BUFFER_SIZE> g_cpuBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuSparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuBadgeFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_ramBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_ramBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_ramBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_ramBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_ramSparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_ramBadgeFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_gpuBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_gpuBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_gpuBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_gpuBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_gpuSparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_gpuBadgeFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_vramBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_vramBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_vramBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_vramBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_vramSparkFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempSparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_cpuTempBadgeFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_batteryBarFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_batteryBar5Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_batteryBar8Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_batteryBar10Formatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_batterySparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_batteryBadgeFormatted;

FormattedString<FORMATTED_BUFFER_SIZE> g_netSparkFormatted;
FormattedString<FORMATTED_BUFFER_SIZE> g_diskSparkFormatted;
...
void DataCollectionSessionInit() {
    ...
    metrics[static_cast<int>(MetricType::kUploadSpeed)] =
        ...
        IsStrInDateTimePatternSettings(L"upload") ||
        IsStrInDateTimePatternSettings(L"net");
    metrics[static_cast<int>(MetricType::kDownloadSpeed)] =
        IsStrInDateTimePatternSettings(L"%download_speed%") ||
        IsStrInDateTimePatternSettings(L"download") ||
        IsStrInDateTimePatternSettings(L"net");

    // If total_speed or net spark/bar is used, we need both upload and download metrics.
    ...
    metrics[static_cast<int>(MetricType::kDiskReadSpeed)] =
        ...
        IsStrInDateTimePatternSettings(L"disk");
    metrics[static_cast<int>(MetricType::kDiskWriteSpeed)] =
        IsStrInDateTimePatternSettings(L"%disk_write%") ||
        IsStrInDateTimePatternSettings(L"disk");

    // If disk_total or disk spark/bar is used, we need both read and write metrics.
    ...
    // percentage is derived from it and the total reported by DXGI.
    ...
    metrics[static_cast<int>(MetricType::kCpuTemp)] =
        ...
        IsStrInDateTimePatternSettings(L"temp");
...
void DataCollectionSampleIfNeeded() {
    ...
    if (g_dataCollectionLastFormatIndex != dataCollectionFormatIndex) {
        if (g_dataCollectionSession) {
            ...
            if (auto cpuVal = g_dataCollectionSession->QueryData(MetricType::kCpu)) {
                g_metricHistories.cpu.Push(*cpuVal);
            }
            if (auto gpuVal = g_dataCollectionSession->QueryData(MetricType::kGpuUsage)) {
                g_metricHistories.gpu.Push(*gpuVal);
            }
            if (auto tempVal = g_dataCollectionSession->QueryData(MetricType::kCpuTemp)) {
                g_metricHistories.temp.Push(*tempVal);
            }
            if (auto usedBytes = g_dataCollectionSession->QueryData(MetricType::kVramUsed)) {
                if (auto totalGb = GetDedicatedVramTotalGb(); totalGb && *totalGb > 0) {
                    double usedGb = *usedBytes / kGBInBytes;
                    g_metricHistories.vram.Push((usedGb / *totalGb) * 100.0);
                }
            }
            auto ul = g_dataCollectionSession->QueryData(MetricType::kUploadSpeed);
            auto dl = g_dataCollectionSession->QueryData(MetricType::kDownloadSpeed);
            if (ul && dl) {
                double totalMbps = ((*ul + *dl) * 8.0) / (1024.0 * 1024.0);
                double netPct = std::min(100.0, (totalMbps / 50.0) * 100.0);
                g_metricHistories.net.Push(netPct);
            }
            auto dr = g_dataCollectionSession->QueryData(MetricType::kDiskReadSpeed);
            auto dw = g_dataCollectionSession->QueryData(MetricType::kDiskWriteSpeed);
            if (dr && dw) {
                double totalMBs = (*dr + *dw) / (1024.0 * 1024.0);
                double diskPct = std::min(100.0, (totalMBs / 100.0) * 100.0);
                g_metricHistories.disk.Push(diskPct);
            }
        }

        if (auto ramStatus = GetRamStatus()) {
            g_metricHistories.ram.Push(static_cast<double>(ramStatus->dwMemoryLoad));
        }

        SYSTEM_POWER_STATUS pwr{};
        if (GetSystemPowerStatus(&pwr)) {
            g_metricHistories.battery.Push(static_cast<double>(pwr.BatteryLifePercent));
        }
...
std::wstring FormatProgressBar(double percent, size_t barLength = 8, BarStyle style = BarStyle::Smooth) {
...
std::wstring FormatSparkline(const RollingMetricHistory& history, size_t requestedLen = 8) {
    if (requestedLen < 2) requestedLen = 2;
    if (requestedLen > 10) requestedLen = 10;
    ...
    if (history.count == 0) {
        for (size_t i = 0; i < requestedLen; i++) {
            result += L' ';
        }
        return result;
    }

    size_t startIdx = (history.count > requestedLen) ? (history.count - requestedLen) : 0;
    size_t sampleCount = history.count - startIdx;

    for (size_t i = 0; i < requestedLen - sampleCount; i++) {
        result += L' ';
    }

    for (size_t i = startIdx; i < history.count; i++) {
        double val = history.samples[i];
        val = std::max(0.0, std::min(100.0, val));
        size_t levelIdx = static_cast<size_t>((val * 7.99) / 100.0);
        if (levelIdx > 7) levelIdx = 7;
        result += levels[levelIdx];
    }

    return result;
}

std::wstring FormatMetricBadge(std::wstring_view metricType, double value) {
    wchar_t buf[64];
    if (metricType == L"temp" || metricType == L"cpu_temp") {
        if (value < 55.0) {
            swprintf_s(buf, L"❄️ %.0f°C", value);
        } else if (value < 75.0) {
            swprintf_s(buf, L"🌡️ %.0f°C", value);
        } else {
            swprintf_s(buf, L"🔥 %.0f°C", value);
        }
    } else if (metricType == L"battery") {
        SYSTEM_POWER_STATUS pwr{};
        bool ac = (GetSystemPowerStatus(&pwr) && pwr.ACLineStatus == 1);
        if (ac) {
...
double GetCpuPercent() {
    DataCollectionSampleIfNeeded();
    if (!g_dataCollectionSession) return 0.0;
    auto val = g_dataCollectionSession->QueryData(MetricType::kCpu);
    return val ? *val : 0.0;
}

double GetRamPercent() {
    auto status = GetRamStatus();
    return status ? static_cast<double>(status->dwMemoryLoad) : 0.0;
}

double GetGpuPercent() {
    DataCollectionSampleIfNeeded();
    if (!g_dataCollectionSession) return 0.0;
    auto val = g_dataCollectionSession->QueryData(MetricType::kGpuUsage);
    return val ? *val : 0.0;
}

double GetVramPercent() {
    DataCollectionSampleIfNeeded();
    if (!g_dataCollectionSession) return 0.0;
    auto usedBytes = g_dataCollectionSession->QueryData(MetricType::kVramUsed);
    auto totalGb = GetDedicatedVramTotalGb();
    if (!usedBytes || !totalGb || *totalGb <= 0) return 0.0;
    double usedGb = *usedBytes / kGBInBytes;
    return (usedGb / *totalGb) * 100.0;
}

double GetCpuTempDouble() {
    DataCollectionSampleIfNeeded();
    if (!g_dataCollectionSession) return 0.0;
    auto val = g_dataCollectionSession->QueryData(MetricType::kCpuTemp);
    return val ? *val : 0.0;
}
...
double GetMetricNumericValue(std::wstring_view metric) {
    if (metric == L"cpu") return GetCpuPercent();
    if (metric == L"ram") return GetRamPercent();
    if (metric == L"gpu") return GetGpuPercent();
    if (metric == L"vram") return GetVramPercent();
    if (metric == L"temp" || metric == L"cpu_temp") return GetCpuTempDouble();
    if (metric == L"battery") return GetBatteryPercent();
    return 0.0;
}

RollingMetricHistory* GetMetricHistory(std::wstring_view metric) {
    if (metric == L"cpu") return &g_metricHistories.cpu;
    if (metric == L"ram") return &g_metricHistories.ram;
    if (metric == L"gpu") return &g_metricHistories.gpu;
    if (metric == L"vram") return &g_metricHistories.vram;
    if (metric == L"temp" || metric == L"cpu_temp") return &g_metricHistories.temp;
    if (metric == L"net" || metric == L"network") return &g_metricHistories.net;
    if (metric == L"disk") return &g_metricHistories.disk;
    if (metric == L"battery") return &g_metricHistories.battery;
    return nullptr;
}

// Cached string getters for CPU
PCWSTR GetCpuBarFormatted() {
    return GetMetricFormatted(g_cpuBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuBar5Formatted() {
    return GetMetricFormatted(g_cpuBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuPercent(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuBar8Formatted() {
    return GetMetricFormatted(g_cpuBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuBar10Formatted() {
    return GetMetricFormatted(g_cpuBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuPercent(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuSparkFormatted() {
    return GetMetricFormatted(g_cpuSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.cpu, 8).c_str());
        return true;
    });
}
PCWSTR GetCpuBadgeFormatted() {
    return GetMetricFormatted(g_cpuBadgeFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatMetricBadge(L"cpu", GetCpuPercent()).c_str());
        return true;
    });
}

// RAM
PCWSTR GetRamBarFormatted() {
    return GetMetricFormatted(g_ramBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetRamPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetRamBar5Formatted() {
    return GetMetricFormatted(g_ramBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetRamPercent(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetRamBar8Formatted() {
    return GetMetricFormatted(g_ramBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetRamPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetRamBar10Formatted() {
    return GetMetricFormatted(g_ramBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetRamPercent(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetRamSparkFormatted() {
    return GetMetricFormatted(g_ramSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.ram, 8).c_str());
        return true;
    });
}
PCWSTR GetRamBadgeFormatted() {
    return GetMetricFormatted(g_ramBadgeFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatMetricBadge(L"ram", GetRamPercent()).c_str());
        return true;
    });
}

// GPU
PCWSTR GetGpuBarFormatted() {
    return GetMetricFormatted(g_gpuBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetGpuPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetGpuBar5Formatted() {
    return GetMetricFormatted(g_gpuBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetGpuPercent(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetGpuBar8Formatted() {
    return GetMetricFormatted(g_gpuBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetGpuPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetGpuBar10Formatted() {
    return GetMetricFormatted(g_gpuBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetGpuPercent(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetGpuSparkFormatted() {
    return GetMetricFormatted(g_gpuSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.gpu, 8).c_str());
        return true;
    });
}
PCWSTR GetGpuBadgeFormatted() {
    return GetMetricFormatted(g_gpuBadgeFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatMetricBadge(L"gpu", GetGpuPercent()).c_str());
        return true;
    });
}

// VRAM
PCWSTR GetVramBarFormatted() {
    return GetMetricFormatted(g_vramBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetVramPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetVramBar5Formatted() {
    return GetMetricFormatted(g_vramBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetVramPercent(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetVramBar8Formatted() {
    return GetMetricFormatted(g_vramBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetVramPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetVramBar10Formatted() {
    return GetMetricFormatted(g_vramBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetVramPercent(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetVramSparkFormatted() {
    return GetMetricFormatted(g_vramSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.vram, 8).c_str());
        return true;
    });
}

// CPU Temp
PCWSTR GetCpuTempBarFormatted() {
    return GetMetricFormatted(g_cpuTempBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuTempDouble(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuTempBar5Formatted() {
    return GetMetricFormatted(g_cpuTempBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuTempDouble(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuTempBar8Formatted() {
    return GetMetricFormatted(g_cpuTempBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuTempDouble(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuTempBar10Formatted() {
    return GetMetricFormatted(g_cpuTempBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetCpuTempDouble(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetCpuTempSparkFormatted() {
    return GetMetricFormatted(g_cpuTempSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.temp, 8).c_str());
        return true;
    });
}
PCWSTR GetCpuTempBadgeFormatted() {
    return GetMetricFormatted(g_cpuTempBadgeFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatMetricBadge(L"temp", GetCpuTempDouble()).c_str());
        return true;
    });
}

// Battery
PCWSTR GetBatteryBarFormatted() {
    return GetMetricFormatted(g_batteryBarFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetBatteryPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetBatteryBar5Formatted() {
    return GetMetricFormatted(g_batteryBar5Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetBatteryPercent(), 5, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetBatteryBar8Formatted() {
    return GetMetricFormatted(g_batteryBar8Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetBatteryPercent(), 8, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetBatteryBar10Formatted() {
    return GetMetricFormatted(g_batteryBar10Formatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatProgressBar(GetBatteryPercent(), 10, BarStyle::Smooth).c_str());
        return true;
    });
}
PCWSTR GetBatterySparkFormatted() {
    return GetMetricFormatted(g_batterySparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.battery, 8).c_str());
        return true;
    });
}
PCWSTR GetBatteryBadgeFormatted() {
    return GetMetricFormatted(g_batteryBadgeFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatMetricBadge(L"battery", GetBatteryPercent()).c_str());
        return true;
    });
}

// Net & Disk Sparklines
PCWSTR GetNetSparkFormatted() {
    return GetMetricFormatted(g_netSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.net, 8).c_str());
        return true;
    });
}
PCWSTR GetDiskSparkFormatted() {
    return GetMetricFormatted(g_diskSparkFormatted, [](PWSTR buf, size_t sz) {
        wcscpy_s(buf, sz, FormatSparkline(g_metricHistories.disk, 8).c_str());
        return true;
    });
}
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        {L"%cpu_bar10%"sv, GetCpuBar10Formatted},
        {L"%cpu_bar5%"sv, GetCpuBar5Formatted},
        {L"%cpu_bar8%"sv, GetCpuBar8Formatted},
        {L"%cpu_bar%"sv, GetCpuBarFormatted},
        {L"%cpu_spark%"sv, GetCpuSparkFormatted},
        {L"%cpu_badge%"sv, GetCpuBadgeFormatted},

        {L"%ram_bar10%"sv, GetRamBar10Formatted},
        {L"%ram_bar5%"sv, GetRamBar5Formatted},
        {L"%ram_bar8%"sv, GetRamBar8Formatted},
        {L"%ram_bar%"sv, GetRamBarFormatted},
        {L"%ram_spark%"sv, GetRamSparkFormatted},
        {L"%ram_badge%"sv, GetRamBadgeFormatted},

        {L"%gpu_bar10%"sv, GetGpuBar10Formatted},
        {L"%gpu_bar5%"sv, GetGpuBar5Formatted},
        {L"%gpu_bar8%"sv, GetGpuBar8Formatted},
        {L"%gpu_bar%"sv, GetGpuBarFormatted},
        {L"%gpu_spark%"sv, GetGpuSparkFormatted},
        {L"%gpu_badge%"sv, GetGpuBadgeFormatted},

        {L"%vram_bar10%"sv, GetVramBar10Formatted},
        {L"%vram_bar5%"sv, GetVramBar5Formatted},
        {L"%vram_bar8%"sv, GetVramBar8Formatted},
        {L"%vram_bar%"sv, GetVramBarFormatted},
        {L"%vram_spark%"sv, GetVramSparkFormatted},

        {L"%cpu_temp_bar10%"sv, GetCpuTempBar10Formatted},
        {L"%cpu_temp_bar5%"sv, GetCpuTempBar5Formatted},
        {L"%cpu_temp_bar8%"sv, GetCpuTempBar8Formatted},
        {L"%cpu_temp_bar%"sv, GetCpuTempBarFormatted},
        {L"%cpu_temp_spark%"sv, GetCpuTempSparkFormatted},
        {L"%cpu_temp_badge%"sv, GetCpuTempBadgeFormatted},
        {L"%temp_bar10%"sv, GetCpuTempBar10Formatted},
        {L"%temp_bar5%"sv, GetCpuTempBar5Formatted},
        {L"%temp_bar8%"sv, GetCpuTempBar8Formatted},
        {L"%temp_bar%"sv, GetCpuTempBarFormatted},
        {L"%temp_spark%"sv, GetCpuTempSparkFormatted},
        {L"%temp_badge%"sv, GetCpuTempBadgeFormatted},

        {L"%battery_bar10%"sv, GetBatteryBar10Formatted},
        {L"%battery_bar5%"sv, GetBatteryBar5Formatted},
        {L"%battery_bar8%"sv, GetBatteryBar8Formatted},
        {L"%battery_bar%"sv, GetBatteryBarFormatted},
        {L"%battery_spark%"sv, GetBatterySparkFormatted},
        {L"%battery_badge%"sv, GetBatteryBadgeFormatted},

        {L"%net_spark%"sv, GetNetSparkFormatted},
        {L"%disk_spark%"sv, GetDiskSparkFormatted},
    ...
    if (format.starts_with(L"%bar:")) {
        size_t closePos = format.find(L'%', 5);
        if (closePos != std::wstring_view::npos) {
            std::wstring_view inner = format.substr(5, closePos - 5);
            std::wstring metric;
            size_t len = 8;
            BarStyle style = BarStyle::Smooth;

            size_t c1 = inner.find(L':');
            if (c1 == std::wstring_view::npos) {
                metric = std::wstring(inner);
            } else {
                metric = std::wstring(inner.substr(0, c1));
                std::wstring_view rest = inner.substr(c1 + 1);
                size_t c2 = rest.find(L':');
                if (c2 == std::wstring_view::npos) {
                    len = _wtoi(std::wstring(rest).c_str());
                } else {
                    len = _wtoi(std::wstring(rest.substr(0, c2)).c_str());
                    std::wstring_view styleStr = rest.substr(c2 + 1);
                    if (styleStr == L"block") style = BarStyle::Block;
                    else if (styleStr == L"pill") style = BarStyle::Pill;
                    else if (styleStr == L"dot") style = BarStyle::Dot;
                }
            }
            if (len < 2) len = 2;
            if (len > 20) len = 20;

            double val = GetMetricNumericValue(metric);
            std::wstring barStr = FormatProgressBar(val, len, style);

            thread_local wchar_t dynBuf[64];
            wcscpy_s(dynBuf, barStr.c_str());
            resolvedCallback(dynBuf);
            return closePos + 1;
        }
    }

    if (format.starts_with(L"%spark:")) {
        size_t closePos = format.find(L'%', 7);
        if (closePos != std::wstring_view::npos) {
            std::wstring_view inner = format.substr(7, closePos - 7);
            std::wstring metric;
            size_t len = 8;
            ...
            if (c1 == std::wstring_view::npos) {
                ...
                metric = std::wstring(inner.substr(0, c1));
                len = _wtoi(std::wstring(inner.substr(c1 + 1)).c_str());
            }
            if (len < 2) len = 2;
            if (len > 10) len = 10;

            RollingMetricHistory* hist = GetMetricHistory(metric);
            std::wstring sparkStr = hist ? FormatSparkline(*hist, len) : std::wstring(len, L' ');

            thread_local wchar_t dynBuf[64];
            wcscpy_s(dynBuf, sparkStr.c_str());
            resolvedCallback(dynBuf);
            return closePos + 1;
        }
    }
...
void ApplyParentHoverShape(FrameworkElement element, int cornerRadius, int customHeight = 0, int customWidth = 0) {
    if (!element) {
        return;
    }
    ...
    for (int i = 0; i < 10; i++) {
        if (!current) break;
        auto frameworkElement = current.try_as<FrameworkElement>();
        if (frameworkElement) {
            frameworkElement.Clip(nullptr);
            if (customHeight > 0) {
                try {
                    frameworkElement.MaxHeight(std::numeric_limits<double>::infinity());
                    frameworkElement.MinHeight((double)customHeight);
                    if (frameworkElement.Height() > 0 && frameworkElement.Height() < customHeight) {
                        frameworkElement.Height((double)customHeight);
                    }
                } catch (...) {}
            }
            if (customWidth > 0) {
                try {
                    frameworkElement.MaxWidth(std::numeric_limits<double>::infinity());
                    frameworkElement.MinWidth((double)customWidth);
                    if (frameworkElement.Width() > 0 && frameworkElement.Width() < customWidth) {
                        frameworkElement.Width((double)customWidth);
                    }
                } catch (...) {}
            }
        }
        if (cornerRadius >= 0) {
            auto control = current.try_as<Controls::Control>();
            if (control) {
                try {
                    control.CornerRadius(winrt::Windows::UI::Xaml::CornerRadius{
                        (double)cornerRadius, (double)cornerRadius,
                        (double)cornerRadius, (double)cornerRadius});
                } catch (...) {}
            }
            auto grid = current.try_as<Controls::Grid>();
            if (grid) {
                try {
                    grid.CornerRadius(winrt::Windows::UI::Xaml::CornerRadius{
                        (double)cornerRadius, (double)cornerRadius,
                        (double)cornerRadius, (double)cornerRadius});
                } catch (...) {}
            }
...
void ApplyContainerStyles(
    FrameworkElement dateTimeIconContentElement,
    Controls::Grid containerGrid,
    Controls::StackPanel stackPanel,
    Controls::TextBlock dateInnerTextBlock,
    Controls::TextBlock timeInnerTextBlock,
    const ContainerStyleSettings* containerStyleSettings) {
    if (!containerGrid) {
        return;
    }

    int targetHeight = (containerStyleSettings && containerStyleSettings->height > 0)
                           ? containerStyleSettings->height
                           : g_settings.height;
    int targetWidth = (containerStyleSettings && containerStyleSettings->width > 0)
                          ? containerStyleSettings->width
                          : g_settings.width;

    if (!containerStyleSettings || !containerStyleSettings->enable) {
        if (targetHeight <= 0 && targetWidth <= 0) {
            containerGrid.as<DependencyObject>().ClearValue(
                Controls::Panel::BackgroundProperty());
            containerGrid.as<DependencyObject>().ClearValue(
                Controls::Grid::BorderBrushProperty());
            containerGrid.as<DependencyObject>().ClearValue(
                Controls::Grid::BorderThicknessProperty());
            containerGrid.as<DependencyObject>().ClearValue(
                Controls::Grid::CornerRadiusProperty());
            containerGrid.as<DependencyObject>().ClearValue(
    ...
    if (containerStyleSettings && containerStyleSettings->ignoreSystemPadding) {
        try {
            if (dateTimeIconContentElement) {
                dateTimeIconContentElement.Margin(winrt::Windows::UI::Xaml::Thickness{0, 0, 0, 0});
                dateTimeIconContentElement.Clip(nullptr);
            }
            if (containerGrid) {
                containerGrid.Margin(winrt::Windows::UI::Xaml::Thickness{0, 0, 0, 0});
                containerGrid.Clip(nullptr);
            }
            if (stackPanel) {
                stackPanel.Margin(winrt::Windows::UI::Xaml::Thickness{0, 0, 0, 0});
            }
            if (dateInnerTextBlock) {
                dateInnerTextBlock.Margin(winrt::Windows::UI::Xaml::Thickness{0, 0, 0, 0});
            }
            if (timeInnerTextBlock) {
                timeInnerTextBlock.Margin(winrt::Windows::UI::Xaml::Thickness{0, 0, 0, 0});
            }
        } catch (...) {}
    }

    int r = (containerStyleSettings && containerStyleSettings->cornerRadius >= 0)
                ? containerStyleSettings->cornerRadius
                : ((containerStyleSettings && containerStyleSettings->glassyFinish) ? 8 : -1);

    if (r >= 0 || targetHeight > 0 || targetWidth > 0) {
        ApplyParentHoverShape(dateTimeIconContentElement, r, targetHeight, targetWidth);
    }
    ...
    if (targetHeight > 0) {
        ...
        try {
            if (dateTimeIconContentElement) {
                dateTimeIconContentElement.Height(h);
                dateTimeIconContentElement.MinHeight(h);
                dateTimeIconContentElement.VerticalAlignment(VerticalAlignment::Center);
            }
            if (containerGrid) {
                containerGrid.Height(h);
                containerGrid.MinHeight(h);
                containerGrid.VerticalAlignment(VerticalAlignment::Center);
            }
        } catch (...) {}
    }
    ...
    if (targetWidth > 0) {
        ...
        try {
            if (dateTimeIconContentElement) {
                dateTimeIconContentElement.Width(w);
                dateTimeIconContentElement.MinWidth(w);
                dateTimeIconContentElement.HorizontalAlignment(HorizontalAlignment::Center);
            }
            if (containerGrid) {
                containerGrid.Width(w);
                containerGrid.MinWidth(w);
                containerGrid.HorizontalAlignment(HorizontalAlignment::Center);
            }
        } catch (...) {}
    }

    if (*containerStyleSettings->backgroundColor) {
        try {
            auto bgColor =
                Markup::XamlBindingHelper::ConvertValue(
                    winrt::xaml_typename<winrt::Windows::UI::Color>(),
                    winrt::box_value(containerStyleSettings->backgroundColor.get()))
                    .as<winrt::Windows::UI::Color>();
            containerGrid.Background(Media::SolidColorBrush{bgColor});
        } catch (...) {}
    } else {
        containerGrid.as<DependencyObject>().ClearValue(
            Controls::Panel::BackgroundProperty());
    ...
    if (*containerStyleSettings->borderColor) {
        try {
            auto borderColor =
                Markup::XamlBindingHelper::ConvertValue(
                    winrt::xaml_typename<winrt::Windows::UI::Color>(),
                    winrt::box_value(containerStyleSettings->borderColor.get()))
                    .as<winrt::Windows::UI::Color>();
            containerGrid.BorderBrush(Media::SolidColorBrush{borderColor});
        } catch (...) {}
    } else {
        containerGrid.as<DependencyObject>().ClearValue(
            Controls::Grid::BorderBrushProperty());
    ...
    if (containerStyleSettings->borderThickness > 0) {
        double t = containerStyleSettings->borderThickness;
        containerGrid.BorderThickness(winrt::Windows::UI::Xaml::Thickness{t, t, t, t});
    } else {
        containerGrid.as<DependencyObject>().ClearValue(
            Controls::Grid::BorderThicknessProperty());
    }

    if (containerStyleSettings->cornerRadius >= 0) {
        double r = containerStyleSettings->cornerRadius;
        containerGrid.CornerRadius(winrt::Windows::UI::Xaml::CornerRadius{r, r, r, r});
    } else {
        containerGrid.as<DependencyObject>().ClearValue(
            Controls::Grid::CornerRadiusProperty());
    }

    if (containerStyleSettings->paddingLeft >= 0 ||
        containerStyleSettings->paddingTop >= 0 ||
        containerStyleSettings->paddingRight >= 0 ||
        containerStyleSettings->paddingBottom >= 0) {
        containerGrid.Padding(winrt::Windows::UI::Xaml::Thickness{
            (double)std::max(0, containerStyleSettings->paddingLeft),
            (double)std::max(0, containerStyleSettings->paddingTop),
            (double)std::max(0, containerStyleSettings->paddingRight),
            (double)std::max(0, containerStyleSettings->paddingBottom)});
    } else {
        containerGrid.as<DependencyObject>().ClearValue(
            Controls::Grid::PaddingProperty());
    }
...
void ApplyDateTimeIconContentStyles(
    ...
    ApplyContainerStyles(
        ...
        dateInnerTextBlock,
        timeInnerTextBlock,
        clockElementStyleEnabled ? &g_settings.containerStyle : nullptr);
...
void LoadSettings() {
    ...
    g_settings.containerStyle.enable =
        Wh_GetIntSetting(L"ContainerStyle.Enable");
    g_settings.containerStyle.backgroundColor =
        StringSetting::make(L"ContainerStyle.BackgroundColor");
    g_settings.containerStyle.borderColor =
        StringSetting::make(L"ContainerStyle.BorderColor");
    g_settings.containerStyle.borderThickness =
        Wh_GetIntSetting(L"ContainerStyle.BorderThickness");
    g_settings.containerStyle.cornerRadius =
        Wh_GetIntSetting(L"ContainerStyle.CornerRadius");
    g_settings.containerStyle.height =
        Wh_GetIntSetting(L"ContainerStyle.Height");
    g_settings.containerStyle.width =
        Wh_GetIntSetting(L"ContainerStyle.Width");
    g_settings.containerStyle.paddingLeft =
        Wh_GetIntSetting(L"ContainerStyle.PaddingLeft");
    g_settings.containerStyle.paddingTop =
        Wh_GetIntSetting(L"ContainerStyle.PaddingTop");
    g_settings.containerStyle.paddingRight =
        Wh_GetIntSetting(L"ContainerStyle.PaddingRight");
    g_settings.containerStyle.paddingBottom =
        Wh_GetIntSetting(L"ContainerStyle.PaddingBottom");
    g_settings.containerStyle.ignoreSystemPadding =
        Wh_GetIntSetting(L"ContainerStyle.IgnoreSystemPadding");
    g_settings.containerStyle.glassyFinish =
        Wh_GetIntSetting(L"ContainerStyle.GlassyFinish");

    g_clockElementStyleEnabled =
        (g_settings.width > 0 || g_settings.height > 0 || g_settings.maxWidth || g_settings.textSpacing ||
         g_settings.containerStyle.enable ||
         *g_settings.containerStyle.backgroundColor ||
         *g_settings.containerStyle.borderColor ||
         g_settings.containerStyle.borderThickness > 0 ||
         g_settings.containerStyle.cornerRadius >= 0 ||
         g_settings.containerStyle.height > 0 ||
         g_settings.containerStyle.width > 0 ||
         g_settings.timeStyle.hidden ||
         *g_settings.timeStyle.textColor ||
